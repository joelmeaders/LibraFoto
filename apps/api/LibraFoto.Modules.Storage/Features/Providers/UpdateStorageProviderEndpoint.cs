...
/// </summary>
public sealed class UpdateStorageProviderEndpoint : Endpoint<UpdateStorageProviderRequestWrapper, Results<Ok<StorageProviderDto>, NotFound<ApiError>, BadRequest<ApiError>>>
{
    ...
    {
        Put("/api/admin/storage/providers/{id:long}");
        Tags("Storage Providers");
        Summary(s =>
        {
            s.Summary = "Update a storage provider";
            s.Description = "Updates an existing storage provider configuration.";
        });
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (entity.Type != StorageProviderType.Local)
        {
            try
            {
                var provider = await factory.GetProviderAsync(request.Id, cancellationToken);
                if (provider != null)
                {
...
public sealed record UpdateStorageProviderRequestWrapper : UpdateStorageProviderRequest
{
    [BindFrom("id")]
