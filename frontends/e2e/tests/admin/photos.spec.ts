...
test.describe.serial("Admin Frontend - Photo Bulk Operations", () => {
  ...
  test("should bulk add photos to album", async ({ page, api }) => {
    ...
    const result = await api.addPhotosToAlbum(testAlbumId, photoIds);
  ...
  test("should bulk add tags to photos", async ({ api }) => {
    ...
    // Add tag to photos
    const result = await api.addTagsToPhotos([testTagId], photoIds);
    expect(result).not.toBeNull();
  ...
  test("should bulk remove photos from album", async ({ api }) => {
    ...
    const result = await api.removePhotosFromAlbum(testAlbumId, photoIds);
    expect(result).not.toBeNull();
  ...
  test("should bulk remove tags from photos", async ({ api }) => {
    ...
    const result = await api.removeTagsFromPhotos([testTagId], photoIds);
    expect(result).not.toBeNull();
