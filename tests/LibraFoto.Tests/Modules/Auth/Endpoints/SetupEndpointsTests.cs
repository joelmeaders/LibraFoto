...
using LibraFoto.Modules.Auth.Features.Setup;
...
public class SetupEndpointsTests
{
    ...
    public async Task GetSetupStatus_WhenSetupRequired_ReturnsOkWithStatus()
    {
        ...
        // Act
        var result = await GetSetupStatusEndpoint.HandleRequestAsync(_setupService, CancellationToken.None);
    ...
    public async Task CompleteSetup_WithValidRequest_ReturnsOkWithLoginResponse()
    {
        ...
        // Act
        var result = await CompleteSetupEndpoint.HandleRequestAsync(request, _setupService, CancellationToken.None);
