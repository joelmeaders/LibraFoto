...
using LibraFoto.Modules.Display.Features.Slideshow;
...
internal static class SlideshowEndpoints_TestHelper
{
    ...
    {
        var request = new GetNextPhotoRequest { SettingsId = settingsId };
        return await GetNextPhotoEndpoint.HandleRequestAsync(request, service, CancellationToken.None);
    }

    public static async Task<Results<Ok<PhotoDto>, NotFound<ApiError>>> GetCurrentPhoto(
        long? settingsId, ISlideshowService service)
    {
        var request = new GetCurrentPhotoRequest { SettingsId = settingsId };
        return await GetCurrentPhotoEndpoint.HandleRequestAsync(request, service, CancellationToken.None);
    }

    public static async Task<Ok<IReadOnlyList<PhotoDto>>> GetPreloadPhotos(
        int? count, long? settingsId, ISlideshowService service)
    {
        var request = new GetPreloadPhotosRequest { Count = count, SettingsId = settingsId };
        return await GetPreloadPhotosEndpoint.HandleRequestAsync(request, service, CancellationToken.None);
    }

    public static async Task<Ok<PhotoCountResponse>> GetPhotoCount(
        long? settingsId, ISlideshowService service)
    {
        var request = new GetDisplayPhotoCountRequest { SettingsId = settingsId };
        return await GetDisplayPhotoCountEndpoint.HandleRequestAsync(request, service, CancellationToken.None);
    }

    public static Ok<ResetResponse> ResetSequence(
        long? settingsId, ISlideshowService service)
    {
        var request = new ResetSlideshowSequenceRequest { SettingsId = settingsId };
        return ResetSlideshowSequenceEndpoint.HandleRequest(request, service);
    }
}
