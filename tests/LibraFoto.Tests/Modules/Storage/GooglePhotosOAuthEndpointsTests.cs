...
using LibraFoto.Modules.Storage.Features.OAuth;
...
public class GooglePhotosOAuthEndpointsTests
{
    ...
    {
        var method = typeof(GetGooglePhotosAuthUrlEndpoint)
    ...
    private static async Task<Results<Ok<StorageProviderDto>, BadRequest<ApiError>, NotFound<ApiError>>> InvokeHandleOAuthCallback(
        ...
        CancellationToken cancellationToken = default)
    {
        var method = typeof(GooglePhotosOAuthCallbackEndpoint)
            .GetMethod("HandleRequestAsync", BindingFlags.NonPublic | BindingFlags.Static);
        ...
        var task = (Task<Results<Ok<StorageProviderDto>, BadRequest<ApiError>, NotFound<ApiError>>>)method
            .Invoke(null, new object[] { providerId, request.AuthorizationCode, dbContext, configuration, loggerFactory, cancellationToken })!;
